// Auto-generated file - do not edit manually
// This file imports all markdown content at build time

import activityMd from '../content/projects/activity.md?raw'
import aetherMd from '../content/projects/aether.md?raw'
import chatBotMd from '../content/projects/chat-bot.md?raw'
import smartSpeakerMd from '../content/projects/smart-speaker.md?raw'
import beaconsMd from '../content/projects/beacons.md?raw'
import birdFmMd from '../content/projects/bird-fm.md?raw'
import boxnetMd from '../content/projects/boxnet.md?raw'
import breakingNewsMd from '../content/projects/breaking-news.md?raw'
import carbonMd from '../content/projects/carbon.md?raw'
import cinemaPosterMd from '../content/projects/cinema-poster.md?raw'
import citiesUnlockedMd from '../content/projects/cities-unlocked.md?raw'
import colorMapperMd from '../content/projects/color-mapper.md?raw'
import colourDetectionMd from '../content/projects/colour-detection.md?raw'
import concreteMd from '../content/projects/concrete.md?raw'
import critCardMd from '../content/projects/crit-card.md?raw'
import datacatcherMd from '../content/projects/datacatcher.md?raw'
import debriefSpyscapeMd from '../content/projects/debrief-spyscape.md?raw'
import drawingRobotMd from '../content/projects/drawing-robot.md?raw'
import elatusMd from '../content/projects/elatus.md?raw'
import encryptionOpsSpyscapeMd from '../content/projects/encryption-ops-spyscape.md?raw'
import figmaUiToolkitMd from '../content/projects/figma-ui-toolkit.md?raw'
import figmailMd from '../content/projects/figmail.md?raw'
import gifGeneratorMd from '../content/projects/gif-generator.md?raw'
import giftMd from '../content/projects/gift.md?raw'
import googlePlayMd from '../content/projects/google-play.md?raw'
import handbellHeroMd from '../content/projects/handbell-hero.md?raw'
import helvarMd from '../content/projects/helvar.md?raw'
import instrument_1Md from '../content/projects/instrument-1.md?raw'
import intellWonderwallMd from '../content/projects/intell-wonderwall.md?raw'
import interrogationOpsSpyscapeMd from '../content/projects/interrogation-ops-spyscape.md?raw'
import keyshotMd from '../content/projects/keyshot.md?raw'
import lightPaintingMd from '../content/projects/light-painting.md?raw'
import lightMd from '../content/projects/light.md?raw'
import magicWandMd from '../content/projects/magic-wand.md?raw'
import magnetismMd from '../content/projects/magnetism.md?raw'
import markdownFeaturesDemoMd from '../content/projects/markdown-features-demo.md?raw'
import measurementsMd from '../content/projects/measurements.md?raw'
import motionStudy_1Md from '../content/projects/motion-study-1.md?raw'
import notebooksMd from '../content/projects/notebooks.md?raw'
import objective_4xMd from '../content/projects/objective-4x.md?raw'
import observerMd from '../content/projects/observer.md?raw'
import particlesMd from '../content/projects/particles.md?raw'
import patternTool_4Md from '../content/projects/pattern-tool-4.md?raw'
import physicalOnenoteMd from '../content/projects/physical-onenote.md?raw'
import presenceMd from '../content/projects/presence.md?raw'
import protopieMd from '../content/projects/protopie.md?raw'
import realTimeClockMd from '../content/projects/real-time-clock.md?raw'
import rfidRingsMd from '../content/projects/rfid-rings.md?raw'
import rfidSpyscapeMd from '../content/projects/rfid-spyscape.md?raw'
import sfSymbolCreatorMd from '../content/projects/sf-symbol-creator.md?raw'
import skewMd from '../content/projects/skew.md?raw'
import slugAndChipMd from '../content/projects/slug-and-chip.md?raw'
import socialRadioMd from '../content/projects/social-radio.md?raw'
import springTableMd from '../content/projects/spring-table.md?raw'
import spyscapeSpecialOpsMd from '../content/projects/spyscape-special-ops.md?raw'
import spyscapeMd from '../content/projects/spyscape.md?raw'
import statusBoardMd from '../content/projects/status-board.md?raw'
import storymakerStorytellerPortfolioMd from '../content/projects/storymaker-storyteller-portfolio.md?raw'
import storymakerStorytellerMd from '../content/projects/storymaker-storyteller.md?raw'
import surveillanceOpsSpyscapeMd from '../content/projects/surveillance-ops-spyscape.md?raw'
import swisscomMd from '../content/projects/swisscom.md?raw'
import switchboardMd from '../content/projects/switchboard.md?raw'
import syntaxHighlighterMd from '../content/projects/syntax-highlighter.md?raw'
import teacakeMd from '../content/projects/teacake.md?raw'
import testCalloutsMd from '../content/projects/test-callouts.md?raw'
import theAlarmingClockMd from '../content/projects/the-alarming-clock.md?raw'
import themeImporterMd from '../content/projects/theme-importer.md?raw'
import topHatMd from '../content/projects/top-hat.md?raw'
import toroidMd from '../content/projects/toroid.md?raw'
import universalCompassMd from '../content/projects/universal-compass.md?raw'
import vitraMd from '../content/projects/vitra.md?raw'
import vsCodeCodiconsMd from '../content/projects/vs-code-codicons.md?raw'
import wearableGesturesMd from '../content/projects/wearable-gestures.md?raw'
import weatherStationMd from '../content/projects/weather-station.md?raw'
import wordCloudsMd from '../content/projects/word-clouds.md?raw'

export const MARKDOWN_CONTENT: Record<string, string> = {
  'activity': activityMd,
  'aether': aetherMd,
  'chat-bot': chatBotMd,
  'smart-speaker': smartSpeakerMd,
  'beacons': beaconsMd,
  'bird-fm': birdFmMd,
  'boxnet': boxnetMd,
  'breaking-news': breakingNewsMd,
  'carbon': carbonMd,
  'cinema-poster': cinemaPosterMd,
  'cities-unlocked': citiesUnlockedMd,
  'color-mapper': colorMapperMd,
  'colour-detection': colourDetectionMd,
  'concrete': concreteMd,
  'crit-card': critCardMd,
  'datacatcher': datacatcherMd,
  'debrief-spyscape': debriefSpyscapeMd,
  'drawing-robot': drawingRobotMd,
  'elatus': elatusMd,
  'encryption-ops-spyscape': encryptionOpsSpyscapeMd,
  'figma-ui-toolkit': figmaUiToolkitMd,
  'figmail': figmailMd,
  'gif-generator': gifGeneratorMd,
  'gift': giftMd,
  'google-play': googlePlayMd,
  'handbell-hero': handbellHeroMd,
  'helvar': helvarMd,
  'instrument-1': instrument_1Md,
  'intell-wonderwall': intellWonderwallMd,
  'interrogation-ops-spyscape': interrogationOpsSpyscapeMd,
  'keyshot': keyshotMd,
  'light-painting': lightPaintingMd,
  'light': lightMd,
  'magic-wand': magicWandMd,
  'magnetism': magnetismMd,
  'markdown-features-demo': markdownFeaturesDemoMd,
  'measurements': measurementsMd,
  'motion-study-1': motionStudy_1Md,
  'notebooks': notebooksMd,
  'objective-4x': objective_4xMd,
  'observer': observerMd,
  'particles': particlesMd,
  'pattern-tool-4': patternTool_4Md,
  'physical-onenote': physicalOnenoteMd,
  'presence': presenceMd,
  'protopie': protopieMd,
  'real-time-clock': realTimeClockMd,
  'rfid-rings': rfidRingsMd,
  'rfid-spyscape': rfidSpyscapeMd,
  'sf-symbol-creator': sfSymbolCreatorMd,
  'skew': skewMd,
  'slug-and-chip': slugAndChipMd,
  'social-radio': socialRadioMd,
  'spring-table': springTableMd,
  'spyscape-special-ops': spyscapeSpecialOpsMd,
  'spyscape': spyscapeMd,
  'status-board': statusBoardMd,
  'storymaker-storyteller-portfolio': storymakerStorytellerPortfolioMd,
  'storymaker-storyteller': storymakerStorytellerMd,
  'surveillance-ops-spyscape': surveillanceOpsSpyscapeMd,
  'swisscom': swisscomMd,
  'switchboard': switchboardMd,
  'syntax-highlighter': syntaxHighlighterMd,
  'teacake': teacakeMd,
  'test-callouts': testCalloutsMd,
  'the-alarming-clock': theAlarmingClockMd,
  'theme-importer': themeImporterMd,
  'top-hat': topHatMd,
  'toroid': toroidMd,
  'universal-compass': universalCompassMd,
  'vitra': vitraMd,
  'vs-code-codicons': vsCodeCodiconsMd,
  'wearable-gestures': wearableGesturesMd,
  'weather-station': weatherStationMd,
  'word-clouds': wordCloudsMd
}
